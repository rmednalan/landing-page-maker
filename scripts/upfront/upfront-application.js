...
var LayoutEditorSubapplication = Subapplication.extend({
	...
	_save_layout_meta: function (preferred_layout, publish) {
		...
		Upfront.Util.post({
			...
			.success(function () {
				...
				setTimeout(function(){
					Upfront.Application.load_layout(_upfront_post_data.layout);//save layout meta
					Upfront.Events.trigger("command:layout:save_success");
				},100);
	...
	_delete_layout: function () {
		...
		Upfront.Util.post({
			...
			.done(function () {
				Upfront.Events.trigger("command:layout:save_success");
				Upfront.Application.load_layout(_upfront_post_data.layout); //delete layout 
			})
		;
	},
	...
	_reset_changes: function () {
		...
		Upfront.Util.post({
			...
			.done(function () {
				Upfront.Events.trigger("command:layout:save_success");
				Upfront.Application.load_layout(_upfront_post_data.layout); //reset 
			})
		;
	},
	...
	create_post: function(postType){
		Upfront.Settings.LayoutEditor.newpostType = postType; 		console.log("check if used in create post");
	...
	createLightboxRegion: function(regionName){
		console.log("create light box region");
		var regions = (
...
var Application = new (Backbone.Router.extend({
	...
	boot: function () {
		...
		if($("#page").hasClass("jwpb-editmode")){
			//alert("upfront load");
	...
	start: function (mode) {
		...
		console.log("is is this layout true");
		if (!!this.layout) {
		...
		{
			console.log("not getting regions");
		}
		...
		if($("#page").hasClass("jwpb-editmode")){
			...
			app.loading.on_finish(function(){
				console.log("finish loaded here");
				console.log(Upfront.Settings.LayoutEditor.Selectors.sidebar);
		...
		require(
			...
			function (objects) {
				...
				app.saveCache = true;
				app.load_layout(_upfront_post_data.layout); console.log("started in start");
				console.log(_upfront_post_data.layout);
				console.log("layout ids");
	...
	load_layout: function (layout_ids, additional) {
		console.log(this.actions.load);
		console.log(additional);
		console.log(layout_ids);
		console.log("load layout action");
		var app = this,
			request_data = {
		...
		console.log("load layout request");

		this.loadingLayout = Upfront.Util.post(request_data)
			.success(function (response) {
				console.log(response);
				console.log("load layout response");
	...
	create_layout: function (layout_ids, additional) {
		console.log(layout_ids);
		console.log("been here in create layout --- ");
		var app = this,
			request_data = {
	...
	set_layout_up: function(layoutData){//from upfront layout response
		var me = this,
		...
		console.log(layoutData.data.post);
		console.log("layout data post");
		...
		console.log(this.layout);
		console.log("upfront models layout")
		...
		console.log(me.layout_view);
		console.log(Upfront.Events);
		console.log("load layout view");
	...
	create_new_entry: function(post_type){
		console.log("here loads the child theme");
		var me= this,
			layoutOps = {
