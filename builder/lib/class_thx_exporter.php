...
class Thx_Exporter {
	...
	private function _set_up_theme_settings () {
		...
		$settings_file = empty($this->_theme) || in_array($this->_theme, array($parent_name->stylesheet, 'theme'))
...
*/
	...
	function inject_dependencies () {
		...
		Thx_Template::plugin()->load('dependencies', array(
			...
			'admin_url' => admin_url('admin.php?page=jwpb-builder'),
	...
	protected function _get_themes () {
		...
		foreach(wp_get_themes() as $index=>$theme) {
			$parent_name = wp_get_theme();
			if ($theme->get('Template') !== $parent_name->stylesheet) continue;
	...
	public function json_export_element_styles () {
		...
		$parent_name = wp_get_theme();
		$stylesheet = !empty($_POST['stylesheet']) && $parent_name->stylesheet !== $_POST['stylesheet']
			? $_POST['stylesheet']
			: false
		;
	...
	public function json_skip_getting_started () {
		...
		if(!class_exists('Thx_Exposed')){
			require_once dirname(__FILE__) . '/class_thx_exposed.php';
		}
		...
		if( is_array( $menu_items ) ){
			foreach($menu_items as $menu_item) {
				...
				if (strpos($menu_item->url, 'create_new') !== false) {
					...
					//echo $menu_item->url."the menu URL";
					continue;
	...
	private function _create_functions_file ($slug) {
		...
		$archive = $this->_template('archive', $data);
		$not_found = $this->_template('404', $data);
		$search = $this->_template('search', $data);
		
		$this->_fs->write(array('archive.php'),$archive);
		$this->_fs->write(array('404.php'),$not_found);
		$this->_fs->write(array('search.php'),$search);
	...
	private function _get_theme_form_defaults ($form=array()) {
		...
		$form = wp_parse_args($form, array(
			'thx-theme-template' => $current_theme->stylesheet,
