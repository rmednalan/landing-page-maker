...
include_once ABSPATH . 'wp-content/plugins/jwpb/library/upfront_functions.php';
include_once ABSPATH . 'wp-content/plugins/jwpb/library/class_upfront_debug.php';
include_once ABSPATH . 'wp-content/plugins/jwpb/library/class_upfront_server.php';
include_once ABSPATH . 'wp-content/plugins/jwpb/library/class_upfront_theme.php';
...
function jwpb_enqueue_styles() {
	// enqueue parent style
	$curr_link = $_SERVER['REQUEST_URI'];
	$request = array_map('trim', explode('/', $curr_link));
	if(!in_array("create_new",$request) and !is_404() and !is_archive() and get_option("show_on_front") == "posts" and !is_search()){ 
		wp_enqueue_style('parent-theme', get_template_directory_uri() .'/style.css');
	}
}
add_action('wp_enqueue_scripts', 'jwpb_enqueue_styles'); 