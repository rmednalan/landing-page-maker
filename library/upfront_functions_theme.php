...
function upfront_locate_template ($template_names) {
	...
	foreach ((array)$template_names as $template_name) {
		...
		if(in_array("create_new",$request)){
			$curr_theme = wp_get_theme();
			$upfront_stylesheet = $curr_theme->stylesheet;
		}
		if(file_exists(WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/' .$template_name)){
			$located = WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/' . $template_name;
			//echo "here";
			break;
		} else if (file_exists(Upfront::get_root_dir() . '/' . $template_name)) {
			$located = Upfront::get_root_dir() . '/'. $template_name;
