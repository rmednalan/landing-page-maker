...
class Upfront_ModuleLoader {
	...
	public function init_core_elements () {
		$core_path = trailingslashit(Upfront::get_root_dir()). 'elements';
		if (!is_dir($core_path)) return false;
		$elements = glob("{$core_path}/*");//find files inside the element directory
	...
	private function _load_element ($element) {
		...
		$entry_points = glob("{$element}/*.php");//loader.php is the entry points
		...
		if (apply_filters('upfront-core_modules-load_module', true, $element_path)) {
			require_once($element_path);//load loaders
		}
		return true;
	}
