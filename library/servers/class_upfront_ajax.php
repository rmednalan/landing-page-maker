...
class Upfront_Ajax extends Upfront_Server {
	...
	function load_page_layout () {
		...
		if ( $post_id ) $post = get_post($post_id);//4

		if (empty($layout_ids))
			$this->_out(new Upfront_JsonResponse_Error("No such layout"));

		upfront_switch_stylesheet($stylesheet);//global upfront stylesheet

		if(is_string($layout_ids)){//definitely not string if child theme
			$layout_ids = Upfront_EntityResolver::ids_from_url($layout_ids);
		}

		if( !$upfront_ajax_query ) $upfront_ajax_query = false;//false
		...
		$page_template_obj = $this->_load_page_template($_POST);//false
	...
	private function _load_page_template ($data) {
		$post_id = (isset($data['post_id'])) ? (int)$data['post_id'] : false;//4
	...
	function get_scoped_regions () {
		$storage_key = $_POST['storage_key'];//theme name : roses
