...
function upfront_element_relative_path ($relpath, $filepath) {
	$templatepath = str_replace('|/+|','/',str_replace('\\','/',Upfront::get_root_dir())); // normalize slashes
...
function upfront_switch_stylesheet ($stylesheet) {
	...
	// echo "<pre>";
	// print_r($stylesheet."-param");
	// print_r($upfront_stylesheet."-global");
	// echo "</pre>.switscfasdfasdfas"; 
...
function upfront_ajax_init () {
	...
	/* if(!class_exists("Upfront_Layout")) require_once(Upfront::get_root_dir()."/library/class_upfront_model.php"); */
...
function upfront_get_attachment_image_lazy ($attachment_id, $ref_size = 'full') {
	...
	$out = '<img class="upfront-image-lazy" src="' . Upfront::get_root_url() . '/img/blank.gif" width="' . $ref_src[1] . '" height="' . $ref_src[2] . '" alt="' . $alt . '" ';
