...
class Upfront_Theme {
	...
	protected function find_default_layout($cascade, $layout_slug = "") {
		...
		$upfront_stylesheet = $child_data->stylesheet;
		
		if(is_child_theme()){
			if( $cascade["specificity"] == "archive"){
				if(file_exists(WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/archive.php')){
					return WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/archive.php';
				}
			}
		}
		
	
		if(is_child_theme() and !in_array("create_new",$request)){
			
			if($cascade["item"] == "single-404_page" ){
				if(file_exists(WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/'.$cascade["item"].'.php')){
					return WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/'.$cascade["item"].'.php';
				}
			}else if($cascade["item"] == "archive-search" ){
				if(file_exists(WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/'.$cascade["item"].'.php')){
					return WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/'.$cascade["item"].'.php';
				}
				
			}else if($cascade["specificity"] == "archive-home"){
				return WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/archive-home.php';
			}else if( $cascade["type"] == "archive"){
				if(file_exists(WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/'.$cascade["item"].'.php')){
					return WP_CONTENT_DIR. '/themes/'.$upfront_stylesheet.'/layouts/archive.php';
				}
			}
